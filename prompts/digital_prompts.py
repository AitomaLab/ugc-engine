...
from prompts.script_split import split_items_proportionally as _split_items_proportionally
...
def generate_ultra_prompt(scene_type, ctx, script_override=None, is_last_scene=False):
    ...
    if video_language == 'es':
        ...
        accent_override = english_accent_line(
            ctx.get('language_accent') or ctx.get('accent'),
        )
...
def build_30s(dur, app_clip, ctx, product=None, influencer=None):
    ...
    _accent = spanish_accent_line(ctx.get('language_accent') or ctx.get('accent'), hint_text=ctx.get('hook')) if _video_lang == 'es' else english_accent_line(ctx.get('language_accent') or ctx.get('accent'))
    ...
    if use_composite:
        ...
        nano_banana_prompt = (
            ...
            f"setting: {env}, tidy and clean with premium casual art direction\n"
            f"lighting: soft directional natural window light, subtle shadows for depth, natural highlight roll-off on skin\n"
            f"camera: iPhone 1x aesthetic, stationary POV camera, character does NOT hold the filming camera, locked off, NO camera movement, NO panning, clean but slightly organic composition, naturally blurred background\n"
