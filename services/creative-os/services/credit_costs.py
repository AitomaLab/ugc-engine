...
CREDIT_COSTS = {
    ...
    "wavespeed_kling_element_register": 2,
    ...
    *,
    ...
    if _is_seedance_model(api):
        return get_video_clip_credit_cost(
            "seedance_2_ugc", dur, has_reference=has_reference,
        )
...
def build_credit_cost_catalog(lang: str = "en") -> dict:
    """Human-readable credit catalog. Delegates to ugc_backend when importable."""
    try:
        from ugc_backend.credit_cost_service import build_credit_cost_catalog as _build
        return _build(lang=lang)
    except ImportError:
        pass
